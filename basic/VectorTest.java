...
public class VectorTest {
	...
	public static void main(String[] args) {
	    ...
	    //----------------------------
	    /*
	     * 제네릭 타입(Generic Type) ==> 클래스 내부에서 사용할 데이터타입을 외부에서 지정하는 기법으로 객체를 생성할 때 
	     * <> 괄호안에 그 객체의 내부에서 사용할 데이터의 타입을 지정해 주는 것을 말한다.
	     * 이런식으로 객체를 생성하면 제네릭 타입으로 지정한 종류의 데이터 이외의 다른 데이터는 저장할 수 없다.
	     * 제네릭 타입으로 지정할 수 있는 데이터타입은 '클래스형'이어야 한다.
	    ...
	    v4.add("EEEE");
	    System.out.println("v2의 size : " + v2.size());
	    System.out.println("v4의 size : " + v4.size());
